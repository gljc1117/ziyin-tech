...
export default function Navbar() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-white/15 bg-[#0b2252] text-white">
    <div className="site-container flex h-16 items-center justify-between gap-3">
      <Link href="/" onClick={() => setOpen(false)} className="flex shrink-0 items-center gap-2" aria-label="子殷科技首页"><Image src="/logo.png" width={48} height={40} alt="" className="h-9 w-auto sm:h-10" /><span className="text-base font-semibold sm:text-lg">子殷科技</span></Link>
      ...
      <div className="flex shrink-0 items-center gap-2 whitespace-nowrap xl:hidden"><Link href="/search" className="inline-flex min-h-11 items-center px-2 text-sm">搜索</Link><button ref={trigger} onClick={()=>setOpen(!open)} aria-expanded={open} aria-controls="mobile-navigation" className="min-h-11 rounded-lg border border-white/30 px-3 text-sm sm:px-4">{open ? "关闭" : "菜单"}</button></div>
    </div>
